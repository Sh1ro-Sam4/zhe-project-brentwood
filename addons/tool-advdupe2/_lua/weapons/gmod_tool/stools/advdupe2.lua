...
CreateConVar("AdvDupe2_AllowUploading", "false", {FCVAR_ARCHIVE})

CreateConVar("AdvDupe2_AllowDownloading", "false", {FCVAR_ARCHIVE})

CreateConVar("AdvDupe2_AllowPublicFolder", "false", {FCVAR_ARCHIVE})
...
if (SERVER) then
    CreateConVar("sbox_maxgmod_contr_spawners", 3)
    ...
    function TOOL:LeftClick(trace)
        -- if table.HasValue({"user","helper"},self:GetOwner():GetUserGroup()) and !self:GetOwner():HasGalaxyPlus() then
        -- 	self:GetOwner():ChatPrint("Это доступно для VIP и выше или игроков с Galaxy.PLUS")
        -- 	return
        -- end
        if not self:GetOwner():IsUserGroup('user') and not self:GetOwner():IsUserGroup('helper') or self:GetOwner():HasGalaxyPlus() then
            if (self:GetOwner().NextMerge or 0) > CurTime() then return false end
            self:GetOwner().NextMerge = CurTime() + 5
            ...
            if (ply.AdvDupe2.Pasting or ply.AdvDupe2.Downloading) then
                AdvDupe2.Notify(ply, "Advanced Duplicator 2 занят. Подождите немного. Вызвано это Pasting или Downloading", NOTIFY_ERROR)
        ...
        if not self:GetOwner():IsUserGroup('user') and not self:GetOwner():IsUserGroup('helper') or self:GetOwner():HasGalaxyPlus() then
            ...
            elseif trace.HitNonWorld then
                ...
                if ply.AdvDupe2.Entities then
                    ...
                    if next(Entities) == nil then return true end
        ...
        AdvDupe2.Encode(Tab, AdvDupe2.GenerateDupeStamp(ply), function(data)
            if (game.SinglePlayer()) then
                ...
                timer.Simple(1, function()
                    AdvDupe2.EstablishNetwork(ply, data)
                end)
            end
        end)
    end
    ...
    net.Receive("AdvDupe2_CanAutoSave", function(len, ply, len2)
        ...
        if (game.SinglePlayer()) then
            ...
            if (time < GetConVarNumber("AdvDupe2_AreaAutoSaveTime")) then
                time = GetConVarNumber("AdvDupe2_AreaAutoSaveTime")
            end
        end
...
if (CLIENT) then
    ...
    local function UpdateGhost()
        ...
        if (tobool(GetConVarNumber("advdupe2_original_origin"))) then
            ...
            if (tobool(GetConVarNumber("advdupe2_offset_world"))) then
                UEntAngle = Angle(0, 0, 0)
            end
    ...
    language.Add("Tool.advdupe2.name", "Дубликатор")
    language.Add("Tool.advdupe2.desc", "")
    language.Add("Tool.advdupe2.0", "ЛКМ: Вставить, ПКМ: Скопировать, Secondary+World: Select/Deselect All, Secondary+Shift: Area copy.")
    language.Add("Tool.advdupe2.1", "ЛКМ: Вставить, ПКМ: Скопировать пропы в площади, Secondary+Shift: Cancel.")
    ...
    local function BuildCPanel()
        ...
        if (not file.Exists("advdupe2", "DATA")) then
        ...
        Check:SetText("Учитывать оригинальную позицию дубликата")
        Check:SetTextColor(Color(0,0,0))
        ...
        Check:SetText("Вставить со сваркой")
        Check:SetTextColor(Color(0,0,0))
		...
		Check_1:SetTextColor(Color(0,0,0))
		...
		Check_2:SetTextColor(Color(0,0,0))
		...
		--]]
        Check = vgui.Create("DCheckBoxLabel")
        Check:SetText("Ограниченные копии для сторон")
        Check:SetTextColor(Color(0,0,0))
...
--]]
        ...
        NumSlider:SetText("% предпросмотра:")
        NumSlider.Label:SetTextColor(Color(0,0,0))
        NumSlider:SetMin(0)
        ...
        NumSlider:SetText("Область копирования:")
        NumSlider.Label:SetTextColor(Color(0,0,0))
        NumSlider:SetMin(0)
        ...
        NumSlider:SetToolTip("Копирует все пропы в области")
        CPanel:AddItem(NumSlider)
        local Category1 = vgui.Create("DCollapsibleCategory")
        CPanel:AddItem(Category1)
        Category1:SetLabel("Позиции")
        ...
        Category2:SetLabel("Информация о дубликате")
        ...
        Category4:SetLabel("Автоматическое сохранение области")
    ...
    net.Receive("AdvDupe2_AddGhost", function(len, ply, len2)
        ...
        local gNew = table.insert(AdvDupe2.GhostToSpawn, {
            R = net.ReadBit() == 1,
        ...
        for k = 0, net.ReadInt(8) do
            AdvDupe2.GhostToSpawn[gNew].PhysicsObjects[k] = {
                Angle = net.ReadAngle(),
                Pos = net.ReadVector()
            }
        end
    ...
    function TOOL:DrawToolScreen()
        ...
        draw.SimpleText("Advanced Duplicator 2", "AD2TitleFont", 128, 50, Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
        draw.SimpleText(text, "AD2Font", 128, 128, Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

        if (state ~= 0) then
            draw.RoundedBox(6, 32, 178, 192, 28, Color(255, 255, 255, 150))
            draw.RoundedBox(6, 36, 182, 188 * (AdvDupe2.ProgressBar.Percent / 100), 24, Color(0,189,0))
        elseif (LocalPlayer():KeyDown(IN_USE)) then
            ...
            draw.SimpleText("Height: " .. LocalPlayer():GetInfo("advdupe2_offset_z"), "AD2TitleFont", 25, 160, Color(255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
            draw.SimpleText("Pitch: " .. LocalPlayer():GetInfo("advdupe2_offset_pitch"), "AD2TitleFont", 25, 190, Color(255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
            draw.SimpleText("Yaw: " .. LocalPlayer():GetInfo("advdupe2_offset_yaw"), "AD2TitleFont", 25, 220, Color(255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
        end
    ...
    local GreenSelected = Color(0,189,0)
    ...
    function AdvDupe2.DrawSelectionBox()
        ...
        surface.SetDrawColor(Color(0,189,0))
        --Draw Sides
