...
local function CollapseParentsComplete(node)
	if(not node.ParentNode.ParentNode)then node:SetExpanded(false) return end
...
local function MoveFileClient(node)
	...
	local path2, area = GetNodePath(node)
...
function FOLDER:Paint(w, h)
    if self:GetPaintBackground() then
        ...
        elseif self.lerpHover > 0 then
            draw.RoundedBox(4, 0, 0, w, h, ColorAlpha(COLOR_ACCENT, 20 * self.lerpHover))
        end
    end
end
...
function FOLDER:Init()
    ...
    self.Label:SetFont("ZCity_Fixed_SuperTiny")
...
local function PaintImgBtn(btn, w, h)
    btn.lerpHover = btn.lerpHover or 0
    btn.lerpHover = Lerp(FrameTime() * 15, btn.lerpHover, btn:IsHovered() and 1 or 0)
    if btn.lerpHover > 0 then
        draw.RoundedBox(4, -2, -2, w+4, h+4, ColorAlpha(COLOR_ACCENT, 40 * btn.lerpHover))
    end
end
...
function PANEL:Init()
    ...
    self.Refresh.Paint = PaintImgBtn
    ...
    self.FileName:SetFont("ZCity_Fixed_SuperTiny")
    ...
    self.Desc:SetFont("ZCity_Fixed_SuperTiny")
    ...
    self.Info:SetFont("ZCity_Fixed_SuperTiny")
