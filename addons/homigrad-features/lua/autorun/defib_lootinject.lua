-- lua/autorun/defib_inject.lua

if SERVER then
    AddCSLuaFile()
end

local WEAPON_CLASS = "weapon_defibrilator_homigrad"
...
local function injectLoot()
    ...
    if istable(mode.LootTableStandard) and istable(mode.LootTableStandard[1]) and istable(mode.LootTableStandard[1][2]) then
        changed = addLootWeapon(mode.LootTableStandard[1][2], 3, WEAPON_CLASS) or changed
    end

    if changed then
        print("[DefibInject] Added defib to HMCD loot tables.")
    end
...
local function injectTDMShop()
    ...
    if mode.BuyItems["Medical"]["Defibrilator"] then
        return true
    end

    mode.BuyItems["Medical"]["Defibrilator"] = {
        Type = "Weapon",
    ...
    print("[DefibInject] Added defib to TDM buy menu in realm: " .. (SERVER and "SERVER" or "CLIENT"))
    return true
end
...
local function startInject()
    ...
    local id = "DefibInject_All_Retry_" .. (SERVER and "SV" or "CL")
...
hook.Add("InitPostEntity", "DefibInject_All_InitPostEntity_" .. (SERVER and "SV" or "CL"), function()
