shizlib.table = shizlib.table or {}

function shizlib.table.toKeyVal(tbl)
	local out = {}
...
function shizlib.table.count(tbl, func)
	local count = 0
	for k, v in pairs(tbl) do
...
function shizlib.table.map(tbl, func)
	local out = {}
	for k, v in pairs(tbl) do
		out[k] = func(v, k, tbl)
	end
...
function shizlib.table.mapSequential(tbl, func)
	local out = {}
	local index = 1
...
function shizlib.table.strip(tbl, defaults, depth)
	depth = depth or 1
	...
	for k, default in pairs(defaults) do
		...
		elseif istable(tbl[k]) then
			if istable(default) and table.Count(shizlib.table.diff(tbl[k], default)) == 0 then
				shizlib.table.strip(tbl[k], default, depth - 1)
			else
				tbl[k] = nil
			end
		end
...
function shizlib.table.reduce(tbl, func, initVal)
...
function shizlib.table.find(tbl, func)
	for k, v in pairs(tbl) do
		if func(v, k) then return v, k end
	end
end

function shizlib.table.filter(tbl, func)
	local found = {}
	for k, v in pairs(tbl) do
...
function shizlib.table.filterMultiple(tbl, funcs, limit)
	local out = {}
	local foundAmount = 0
	for k, v in pairs(tbl) do
		if shizlib.array.every(funcs, function(func)
			return func(v, k)
		end) then
			out[k] = v
...
function shizlib.table.precacheFilterFunctions(query)
	local funcs = {}
...
function shizlib.table.filterQuery(tbl, query, limit)
	return shizlib.table.filterMultiple(tbl, shizlib.table.precacheFilterFunctions(query), limit)
end

function shizlib.table.diff(t1, t2, changes)
	changes = changes or {}
	...
	for k, v in pairs(t1) do
		if istable(v) then
			if t2[k] ~= nil then
				local innerChanges = {}
				shizlib.table.diff(v, t2[k], innerChanges)
