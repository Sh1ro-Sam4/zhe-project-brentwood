itemstore.items = itemstore.items or {}
itemstore.items.Registered = itemstore.items.Registered or {}
itemstore.items.Pickups = itemstore.items.Pickups or {}
...
function itemstore.items.Load()
	...
	-- for k, v in pairs( shizlib.Food ) do
	-- 	local ITEM = setmetatable( {}, { __index = Item } )
	-- 	ITEM.Class = 'shizlib_food_' .. string.Replace( string.lower( k ), " ", "" )
	-- 	ITEM.Name = v.name and v.name or 'nil'
	-- 	ITEM.Description = v.description and v.description or ''
	-- 	ITEM.Stackable = true
	-- 	ITEM.HighlightColor = v.color and v.color or itemstore.config.HighlightColours.Food
	-- 	ITEM.Base = "base_auto"
	-- 	itemstore.items.Register( ITEM )
	-- end
	...
	for name, tbl in pairs(hg.ammotypes) do
		local ITEM = setmetatable( {}, { __index = Item } )
	...
	if hg and hg.armor then
		for category, armors in pairs(hg.armor) do
			for armorkey, armorData in pairs(armors) do
				local class = "ent_armor_" .. armorkey
				local ITEM = setmetatable( {}, { __index = Item } )
				ITEM.Class = class
				ITEM.Name = hg.armorNames[armorkey] or armorkey
				ITEM.Description = (category == "head" or category == "face" or category == "ears") and "Шлем / Маска" or "Бронежилет"
				ITEM.Model = armorData[2] or "models/error.mdl"
				ITEM.HighlightColor = itemstore.config.HighlightColours.Other
				ITEM.Base = "base_entity"

				function ITEM:SaveData( ent )
					if ent.mat then
						self:SetData("Material", ent.mat)
					end
					if ent.skin then
						self:SetData("Skin", ent.skin)
					end
					if ent.zablevano then
						self:SetData("Zablevano", ent.zablevano)
					end
				end

				function ITEM:LoadData( ent )
					if self:GetData("Material") then
						ent.mat = self:GetData("Material")
						ent:SetSubMaterial(0, ent.mat)
					end
					if self:GetData("Skin") then
						ent.skin = self:GetData("Skin")
						ent:SetSkin(ent.skin)
					end
					if self:GetData("Zablevano") then
						ent.zablevano = self:GetData("Zablevano")
					end
				end

				function ITEM:Use( pl )
					if CLIENT then return end
					if not pl or not IsValid(pl) or not pl:Alive() then return false end

					local can = hg.AddArmor(pl, armorkey)
					if can then
						if self:GetData("Zablevano") then
							pl:SetNetVar("zableval_masku", true)
						end
						local placement = hg.GetArmorPlacement(armorkey)
						if placement then
							local mat = self:GetData("Material")
							local skin = self:GetData("Skin")
							if mat then
								pl:SetNWString("ArmorMaterials" .. armorkey, mat)
							end
							if skin then
								pl:SetNWInt("ArmorSkins" .. armorkey, skin)
							end
							pl:SyncArmor()
						end
