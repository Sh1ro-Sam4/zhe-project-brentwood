...
if SERVER then
	function PROVIDER:SetMoney( pl, money )
		local currentmoney = pl:GetNWInt("Money", 0)
		local newmoney = math.max(0, money)
		pl:SetNWInt("Money", newmoney)
		SavePlayerMoney(pl)
	end
	...
	if itemstore.config.EnableInvholster then
		itemstore.AddCommand( "invholster", function( pl, args )
			...
			if not IsValid(wep) or not wep:GetModel() or wep:GetModel() == "" then
				DarkRP.notify(pl, 1, 4, "Нельзя выбросить оружие")
				return
			end
			...
			if GAMEMODE.Config.DisallowDrop[ wep:GetClass() ] then
				DarkRP.notify(pl, 1, 4, "Нельзя выбросить оружие")
				return
			end
