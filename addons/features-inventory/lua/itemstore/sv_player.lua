...
function meta:PickupItem( ent )
	...
	ent = ent or hg.eyeTrace(self).Entity

	self:DelayedAction('itempickup' .. ent:EntIndex(), 'Складывание предмета ', {
		time = 3,
		check = function()
			if !IsValid(ent) then return false end
			if not self:CanUseInventory() then return false end
			if ent:GetPos():Distance(self:GetPos()) > 85 then return false end
			if not IsValid( ent ) or ent.__Deleted or not itemstore.items.Pickups[ ent:GetClass() ] then return false end
		...
		inst = true,
		action = function()
            self:EmitSound("player/clothes_generic_foley_0" .. math.random(1,5) .. ".wav")
            self:EmitSound("universal/uni_lean_in_0" .. math.random(1,4) .. ".wav")
		end,
	})
end
...
function meta:DropItem( con_id, slot )
	...
	if item['Data'].EntityData or (item['Data'].Model and item['Data'].Class) then
		...
		if CFG.limitEntity[class] then
			...
			for k, v in ents.Iterator() do
				if v:CPPIGetOwner() == owner and v:GetClass() == class then
					count = count + 1
				end
			end

			if count + 1 > CFG.limitEntity[class] then
				kas.notify(self, Color(189,0,0), 'Инвентарь', string.format( 'Вы достигли лимита %s', class ) )
				return
			end
		end
	else
		...
		if CFG.limitEntity[class] then
			...
			if count + 1 > CFG.limitEntity[class] then
				DarkRP.notify(self, 0, 4, string.format( 'Вы достигли лимита %s', class ) )
				return
			end
		end
	end
	...
	if item.Class == 'base_accessory' then
		kas.notify(self, Color(189,0,0), 'Инвентарь', string.format( 'Энтити: %s невозможно выбросить', item.Class ) )
		return
	end
