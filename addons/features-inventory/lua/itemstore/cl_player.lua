...
net.Receive( "ItemStoreOpen", function()
	...
	ITEMSTORE_TEMP_STORAGE = vgui.Create( "ItemStoreContainerWindow" )

	local panel = ITEMSTORE_TEMP_STORAGE
	...
	if not hideinv then
		...
		function inv:Think()
			think( self )
			if not IsValid(ITEMSTORE_TEMP_STORAGE) then self:Remove() return end
