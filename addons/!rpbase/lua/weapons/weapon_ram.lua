...
function SWEP:PrimaryAttackAdd(ent, trace)
    -- Активация fading doors
    if SERVER and bKeypads and bKeypads.FadingDoors and bKeypads.FadingDoors:IsFadingDoor(ent) then
        ent.fadeActive = not ent.fadeActive
    ...
    if phys and not ent:IsRagdoll() then
        ...
        if SERVER and ent:GetNWBool("fadingfix") and ent.CPPIGetOwner then
        ...
        local pushvec = trace.Normal * 10000
