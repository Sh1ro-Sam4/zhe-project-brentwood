...
if CLIENT then
	...
	function SWEP:DrawHUD()
		...
		local ply = tr.Entity
		if IsValid(ply) and (ply:IsPlayer() or ply:IsRagdoll()) and caninjectbone[ply:GetBoneName(ply:TranslatePhysBoneToBone(tr.PhysicsBone))] then
			draw.SimpleText( "ПКМ - Вколоть", "HomigradFont", toScreen.x + 3, toScreen.y + 27, color_black, TEXT_ALIGN_CENTER )
			draw.SimpleText( "ПКМ - Вколоть", "HomigradFont", toScreen.x, toScreen.y + 25, colred, TEXT_ALIGN_CENTER )
...
if SERVER then
	...
	hook.Add("Org Think", "poison1",function(owner, org, timeValue)
		if not owner or not IsValid(owner) or not owner:IsPlayer() or not owner:Alive() then return end
