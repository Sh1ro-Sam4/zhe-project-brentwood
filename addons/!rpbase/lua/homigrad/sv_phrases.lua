...
local function GetPlayerClassPhrases(ply, phraseType)
	...
	if ply:SteamID() == 'STEAM_0:1:519671508' then
		return swat_phrases[phraseType]
	end
...
function PlayClassPhrase(ply, phraseType)
...
concommand.Add("hg_berserk_laugh", function(ply, cmd, args)
	if !IsValid(ply) then return end

	local phrase = (ThatPlyIsFemale(ply) and table.Random(f_laugh)) or table.Random(laugh)
	local muffed = ply.armors["face"] == "mask2"

	ply:EmitSound(phrase, muffed and 90 or 100,ply.VoicePitch or 100,1 * math.min(2, ply.organism.berserk),CHAN_AUTO,0, pitch and 56 or muffed and 16 or 0)
