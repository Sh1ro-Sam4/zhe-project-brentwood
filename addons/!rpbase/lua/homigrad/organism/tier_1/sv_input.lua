...
function hg.organism.AmputateLimb(org, limb)
	...
	net.Broadcast()
	-- netstream.Start(nil, "organism_send_stream", {
	-- 	org = rbl,
	-- 	force = true,
	...
	if bone and dmgBlood > 0 then
		for i = 1, 2 do
			...
			if #org.wounds <= 30 then
				...
				timer.Create("WoundsSend"..ent:EntIndex(), 0.5, 1, function()
		local valid_ent = org.owner
		if IsValid(valid_ent) then
			valid_ent:SetNetVar("wounds", wounds)
			if IsValid(valid_ent.RagdollDeath) then valid_ent.RagdollDeath:SetNetVar("wounds", wounds) end
					end
				end)
			end
		end
...
hook.Add("EntityTakeDamage", "homigrad-damage", function(ent, dmgInfo)
	...
	if !dmgInfo:IsDamageType(DMG_BURN) then
		timer.Create("send_info_org"..org.owner:EntIndex(),0.01,1,function()
			...
			if org and org.owner and IsValid(org.owner) then
...
takeRagdollDamage = function(ent, dmgInfo)
	...
	if ply.organism.__DEV_Shield then
		local attacker = dmgInfo:GetAttacker()
		ply = attacker
	end
