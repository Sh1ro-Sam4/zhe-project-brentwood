...
if SERVER then
    hook.Remove("PostCleanupMap","huyhuygordonspasjizn",function(ent)
        timer.Simple(1,function()
...
hook.Remove("Player Think","health_armor_gordonthings",function(ply)
...
hook.Remove("WeaponEquip","pickuplom",function(wep,ply)
...
hook.Remove("PlayerCanPickupItem","hevsuit",function(ply, ent)
...
hook.Remove("HomigradDamage","takesomearmor",function(ply,dmginfo,hitgroup,ent,harm)
    if not (ply.PlayerClassName == "Gordon" and ply:GetNetVar("HEVSuit")) then return end
...
if CLIENT then
    ...
    hook.Remove("RenderScreenspaceEffects","HEV_helmet",function()
...
hook.Remove("CanListenOthers", "GordonWeDontHearYou", function(talker)
    if talker:Alive() and talker.PlayerClassName == "Gordon" then
        return false, false 
    end
end)

hook.Remove("HG_PlayerSay", "GordonWeDontSeeYouChat", function(ply, text)
...
if CLIENT then
    ...
    local inPlaying = 0
    hook.Remove("Think","HEV_Notify",function()
        local armors = lply.armors
...
elseif SERVER then
    ...
    hook.Remove("Org Think","gordon_healing",function(ply, org, timeValue)
    ...
    hook.Remove("HomigradDamage", "HEV_Medical",function(ply, dmgInfo, hitgroup, ent, entharm)
        if ply.PlayerClassName == "Gordon" and ply:GetNetVar("HEVSuit") then
