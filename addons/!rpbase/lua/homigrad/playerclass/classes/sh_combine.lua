...
hook.Remove("PostCleanupMap", "CombineSquads_Reset", function()
...
function CLASS.On(self, data)
    ...
    hook.Remove( "OnEntityCreated", "relation_shipdo"..index, function( ent )
...
if CLIENT then
    ...
    hook.Remove("RenderScreenspaceEffects","Combine_helmet",function()
        local lply = LocalPlayer()
...
if SERVER then
    hook.Remove("HG_PlayerFootstep","Combine_footsteps",function(ply)
    ...
    hook.Remove("HomigradDamage","Combine_painsounds",function(ply, dmgInfo, hitgroup, ent)
    ...
    hook.Remove("HGReloading","Combine_reloadalert",function(wep)
    ...
    hook.Remove("HG_PlayerCanHearPlayersVoice","CombineRadio",function(listener,talker)
        if talker.PlayerClassName == "Combine" and listener.PlayerClassName == "Combine" and talker:Alive() then
            return true,false
        end
    end)

    hook.Remove("HG_PlayerSay","CombineChatMessage",function(ply,text)
...
if CLIENT then
    ...
    hook.Remove("PlayerStartVoice","CombineRadioStart",function(ply)
        if ply.PlayerClassName == "Combine" and ply:Alive() then
            ply:EmitSound(radio_end_sound)
        end
    end)
    hook.Remove("PlayerEndVoice","CombineRadioEnd",function(ply)
        if ply.PlayerClassName == "Combine" and ply:Alive() then
            ply:EmitSound(radio_end_sound)
        end
    end)

    hook.Remove("HG_NoSoundproof","CombineNoSoundproof",function(pPly, lply)
...
if CLIENT then
    ...
    local function togglePNV()
        ...
        if pnv_enabled then
            ...
            hook.Remove("RenderScreenspaceEffects","PNV_ColorCorrection",function()
    ...
    hook.Remove("PreDrawHalos","PNV_Light",function()
        local ply = LocalPlayer()
        if ply.PlayerClassName ~= "Combine" then return end
        if pnv_enabled then
    ...
    hook.Remove("Think","PNV_Think",function()
        local ply = LocalPlayer()
        if ply:Alive() and ply.PlayerClassName == "Combine" then
