...
hook.Remove('Think','Swarm',function() 
...
hook.Remove('Think','Swarm2',function()
...
hook.Remove('PlayerDeath','Swarm',function(p) 
...
hook.Remove('Player Spawn','Swarm',function(p)
	-- if(!p:Alive() or !IsValid(p.FakeRagdoll))then
	p:ClearSwarm()
	-- end
end)

hook.Remove('CanPlayerSuicide','Swarm',function(p) 
...
hook.Remove('PostCleanupMap','Swarm',function()
	for i,p in pairs(player.GetAll())do
		p:ClearSwarm()
	end 
end)
...
--hook.Remove('EntityTakeDamage','Swarm',function(p,dmg)if(dmg:GetAttacker():GetClass()=='npc_swarm')then p.SwarmPerc=(p.SwarmPerc or 0)+105 p.Swm=true p.Inf=1 end end)
...
hook.Remove("Think","SWARM_Misc",function()
...
hook.Remove("CalcMainActivity","Swarm",function(ply,vel)
	if(ply.SWARM_KnockoutEnd)then
		return ACT_INVALID,ply:LookupSequence("seq_cower")
	end
end)

hook.Remove("StartCommand","Swarm",function(ply,cmd)
