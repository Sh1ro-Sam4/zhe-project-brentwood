...
--\\SQL
	...
	hook.Remove("DatabaseConnected", "AbnormaltiesSQL", function()
	...
	hook.Remove("PlayerInitialSpawn", "AbnormaltiesSQL", function(ply)
...
--\\Universal save system
	...
	hook.Remove("Think", "Abnormalties_SQLSave", function()
	...
	hook.Remove("PlayerDisconnected", "Abnormalties_SQLSave", function(ply)
		if(ply.AbnormaltiesReady)then
			local steam_id_64 = ply:SteamID64()
...
--\\Consequences
	...
	--=\\Hooks
		...
		hook.Remove("PlayerPostThink", "Abnormalties_Consequences", function(ply)
			if(ply.AbnormaltiesReady)then
...
--\\
	...
	--=\\Testing
		...
		hook.Remove("Abnormalties_HotZoneAbnormaltyAdded", "Abnormalties", function(zone_id, abnormalty_name, amt, ply)
			local zone = PLUGIN.Zones[zone_id]
...
--\\Special Equipment
	hook.Remove("PreHomigradDamage", "Abnormalties_SpecialEquipment", function(ply, dmg, hitgroup, ent, harm)
		if(ply.armors)then
			if(ply.armors["torso"] == "ego_equalizer")then
	...
	hook.Remove("PreTraceOrganBulletDamage", "Abnormalties_SpecialEquipment", function(org, bone, dmg, dmgInfo, box, dir, hit, ricochet, organ, hook_info)
		local ply = org.owner
	...
	hook.Remove("PreHomigradDamageBulletBleedAdd", "Abnormalties_SpecialEquipment", function(ent, org, dmgInfo, hitgroup, harm, hitBoxs, inputHole, hook_info)
		local ply = org.owner
...
--\\??
	...
	hook.Remove("PostCleanupMap", "Abnormalties", function()
		PLUGIN.HotWords = {}
		PLUGIN.Zones = {}
		PLUGIN.HotZones = {}
	...
	hook.Remove("PlayerSay", "Abnormalties", function(ply, text, team_chat)
	...
	hook.Remove("HomigradDamage", "Abnormalties_Equalizers", function(ply, dmg, hitgroup, ent, harm)
	...
	hook.Remove("HG_BloodParticleStartedDropping", "Abnormalties", function(owner, org, wound, dir, artery)
