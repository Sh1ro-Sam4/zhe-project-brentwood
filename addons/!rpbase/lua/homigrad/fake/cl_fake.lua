...
hook.Remove("OnEntityCreated", "TryCopyAppearanceNow", function( ent )
...
-- Ensure custom collision check is enabled for all players on client dynamically
hook.Remove("Think", "FakePlayerCustomCollisionClient", function()
	for _, ply in ipairs(player.GetAll()) do
		if IsValid(ply) and not ply.CustomCollisionSetup then
			ply:SetCustomCollisionCheck(true)
			ply.CustomCollisionSetup = true
		end
		
		local isFaked = ply:GetNWBool("IsFaked")
		if ply.LastFakeState ~= isFaked then
			ply.LastFakeState = isFaked
			ply:CollisionRulesChanged()
			if not isFaked then
				ply:SetHull(Vector(-10, -10, 0), Vector(10, 10, 72))
				ply:SetHullDuck(Vector(-10, -10, 0), Vector(10, 10, 36))
				ply:SetViewOffset(Vector(0, 0, 64))
				ply:SetViewOffsetDucked(Vector(0, 0, 38))
			end
		end
		
		-- Also ensure faked players' properties are forced client-side every frame
		if IsValid(ply) and isFaked then
			ply:SetAvoidPlayers(false)
			ply:SetSolid(SOLID_NONE)
			ply:SetCollisionGroup(COLLISION_GROUP_IN_VEHICLE)
			ply:SetHull(Vector(-0.01, -0.01, 0), Vector(0.01, 0.01, 0.01))
			ply:SetHullDuck(Vector(-0.01, -0.01, 0), Vector(0.01, 0.01, 0.01))
...
-- Ensure solidity and collision group are overridden during player movement simulation to prevent race conditions
hook.Remove("SetupMove", "FakePlayerSetupMoveClient", function(ply, mv, cmd)
	for _, p in ipairs(player.GetAll()) do
		if IsValid(p) and p:GetNWBool("IsFaked") then
			p:SetSolid(SOLID_NONE)
			p:SetCollisionGroup(COLLISION_GROUP_IN_VEHICLE)
			p:SetHull(Vector(-0.01, -0.01, 0), Vector(0.01, 0.01, 0.01))
			p:SetHullDuck(Vector(-0.01, -0.01, 0), Vector(0.01, 0.01, 0.01))
		end
	end
end)

hook.Remove("ShouldCollide", "FakePlayerNoCollide", function(ent1, ent2)
	if not IsValid(ent1) or not IsValid(ent2) then return end

	if ent1:IsPlayer() and ent1:GetNWBool("IsFaked") then
		return false
	end

	if ent2:IsPlayer() and ent2:GetNWBool("IsFaked") then
		return false
	end
end)