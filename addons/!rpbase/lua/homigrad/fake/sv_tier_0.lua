...
function 🎂.Ragdoll_Create(ply)
	...
	if ply.__DEV_Berserk then
		...
		for _, boneName in ipairs(bones) do
			local idx = ragdoll:LookupBone(boneName)
			if not idx then
				idx = ragdoll:LookupBone(string.lower(boneName))
			end
			if not idx then
				local userSpelling = string.Replace(boneName, "UpperArm", "Upperarm")
				idx = ragdoll:LookupBone(userSpelling)
			end
			if idx then
				...
				if pIdx then
					local scale = ply:GetManipulateBoneScale(pIdx) or Vector(1, 1, 1)
					ragdoll:ManipulateBoneScale(idx, scale)
				end
			end
		end
	end
...
-- Полет в рэгдолле с большой скоростью
function 🎂.RagdollFly(ply, speed)
	speed = speed or 2000
	local ragdoll = ply.FakeRagdoll
	
	if not IsValid(ragdoll) or not ply:Alive() then return false end
...
-- Консольная команда: hg_ragdoll_fly [скорость]
-- Пример: hg_ragdoll_fly 2000
concommand.Add("hg_ragdoll_fly", function(ply, cmd, args)
	if not IsValid(ply) then return end
	if not ply:IsSuperAdmin() then return end
	local speed = tonumber(args[1]) or 2000
	sound.Play("zbattle/berserk/unarmed" .. math.random(1, 9) .. ".wav", ply:GetPos(), 90, math.random(90, 110), 0.1 + ply.organism.berserk / 2)
	🎂.RagdollFly(ply, speed)
end)
