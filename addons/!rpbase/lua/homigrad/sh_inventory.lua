...
if CLIENT then
	...
	OpenInv = function(ent)
		...
		local money = (IsValid(ent) and ent.GetMoney) and ' (' .. FormatMoney(ent:GetMoney()) .. ')' or ""
		...
		plyMenu.PaintOver = function(self, w, h)
			...
			draw.DrawText(name .. money, "HomigradFontSmall", w / 2, 10, color_white, TEXT_ALIGN_CENTER)
		...
		timer.Simple(.1, function()
			if IsValid(ITEMSTORE_TEMP_STORAGE) and IsValid(plyMenu) then
				ITEMSTORE_TEMP_STORAGE:SetTitle( ("Инвентарь: %s"):format(name) )
				ITEMSTORE_TEMP_STORAGE:ShowCloseButton( true )
				ITEMSTORE_TEMP_STORAGE:SetDraggable( true )
				ITEMSTORE_TEMP_STORAGE:InvalidateLayout( true )
				ITEMSTORE_TEMP_STORAGE:SetPos( plyMenu:GetPos() + plyMenu:GetWide() + s(10), shizlib.hud.ScrH / 2 - ITEMSTORE_TEMP_STORAGE:GetTall() )
				function ITEMSTORE_TEMP_STORAGE:Think()
					if not IsValid(plyMenu) then self:Remove() return end
				end
			end
		end)
