...
function ENT:Draw()
	...
	local x = math.sin(CurTime() * math.pi) * 60

	cam.Start3D2D((pos + self:GetForward() ), ang, 0.03)
		draw.SimpleTextOutlined('Готовые детали сюда!', '3d2d', 0, x - 150, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	cam.End3D2D()
end