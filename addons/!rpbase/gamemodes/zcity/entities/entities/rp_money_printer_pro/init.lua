...
function ENT:Initialize()
	...
	timer.Create(self:EntIndex() .. 'Print', 60 / self:GetUD_Speed(), 0, function()
		...
		if IsValid(ent) and string.find(ent:GetClass(), "rp_money_printer") then
			if idx == 1 then
				...
				if ent:GetEnabled() then
					...
					timer.Create(ent:EntIndex() .. 'Print', 60 / ent:GetUD_Speed(), 0, function()
...
function ENT:PrintMoney()
	...
	elseif (self:GetHP() <= 0) then
		...
		--end
		self:SetMoneyInMe(self:GetMoneyInMe() + math.random(75,125))
	end
end