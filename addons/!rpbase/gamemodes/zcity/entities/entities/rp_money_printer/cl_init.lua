...
local function DrawTerminalFrame(x, y, w, h, isEnabled)
    ...
    local darkCol = isEnabled and color_dark_green or color_dark_red
...
local target_coin_count = 0
local coin_radius = 75        
local gravity = 400           
...
local function DrawCyberVault(x, y, w, h, money, enabled)
    if enabled then
        for i = 1, #matrix_streams do
            ...
            for j = 1, 5 do
                ...
                draw.SimpleText(c, "Prn_Matrix", char_x, char_y - j*14, Color(0, 255, 60, alpha), TEXT_ALIGN_LEFT)
            end
    ...
    if enabled then
        draw.SimpleText("IMP-MK7-CV-88X", "Prn_Header", x + w/64, y + h - 940, color_main, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
        draw.SimpleText("FW 3.1.5-rc2 (сборка 451)", "Prn_Header", x + w/64, y + h - 880, color_main, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
    else
        draw.SimpleText("IMP-MK7-CV-88X", "Prn_Header", x + w/64, y + h - 940, color_red, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
        draw.SimpleText("FW 3.1.5-rc2 (сборка 451)", "Prn_Header", x + w/64, y + h - 880, color_red, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
    end
    if not enabled then
        local blink = math.sin(CurTime() * 4) > 0
        draw.SimpleText("[ ХРАНИЛИЩЕ ОТКЛЮЧЕНО ]", "Prn_Huge", x + w/2, y + h/2 - 30, blink and color_red or color_dark_red, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    ...
    for i = 1, #coin_particles do
        ...
        surface.SetDrawColor(200, 170, 0, alpha)
        surface.DrawCircle(p.x, p.y, coin_radius - 3)

        draw.SimpleText("$", "Prn_Header", p.x, p.y - 8, Color(180, 130, 0, alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
...
function ENT:Draw()
    ...
    if LocalPlayer():GetPos():DistToSqr(self:GetPos()) > 1500 then return end
    ...
    cam.Start3D2D(pos + ang:Up() * 0.1, ang, 0.01)
        ...
        if isEnabled then
            ...
            local printPerc = math.Clamp((CurTime() - self:GetLastPrint()) / (120 / printSpeed), 0, 1)
            DrawSegmentedBar(barX, oy + 180, barW, barH, printPerc, "> ПРОЦЕСС ПЕЧАТИ:", string.format("%d%%", printPerc * 100), color_main, color_dark_green)
            ...
            local hpDark = hpPerc > 0.3 and color_dark_green or color_dark_red
            ...
            draw.SimpleText("[ ПИТАНИЕ ОТКЛЮЧЕНО ]", "Prn_Huge", cx, cy - 60, blink and color_red or color_dark_red, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
