...
if CLIENT then
    ...
    local function RemoveSandboxTabs()
        local AccsesGroup = {'superadmin', 'curator'}
        local tabstoremove = {
        ...
        if table.HasValue(AccsesGroup, LocalPlayer():GetUserGroup()) or LocalPlayer():HasFullSpawnMenu() then 
            if not LocalPlayer():IsSuperAdmin() and not LocalPlayer():HasFullSpawnMenu() and LocalPlayer():SteamID64() ~= "76561199082487641" then 
