...
hook.Add("radialOptions", "surrender_option", function()
    ...
    if inKneel then
        hg.radialOptions[#hg.radialOptions + 1] = {
                ...
                if mouseClick == 2 then
                    if inHandsBehindHead then
                        hg.CreateRadialMenu({
                            ...
                            { function() ExitHandsBehindHead(ply) end, "Руки вверх (обычно)" },
                        })
                    else
                        hg.CreateRadialMenu({
                            { function() ExitKneel(ply) end,            "Встать"   },
                            { function() EnterHandsBehindHead(ply) end, "Руки за голову" },
                        })
                    end
                    return -1
                else
            ...
            inHandsBehindHead and "На коленях + Руки за головой\nПКМ - Варианты" or "На коленях + Руки вверх\nПКМ - Варианты"
        }
        return
    end
        ...
        hg.radialOptions[#hg.radialOptions + 1] = {
            function(mouseClick)
                if mouseClick == 2 then
                    if inHandsBehindHead then
                        hg.CreateRadialMenu({
                            { function() ExitSurrender(ply) end,       "Перестать сдаваться" },
                            { function() EnterKneel(ply) end,          "Встать на колени"    },
                            { function() ExitHandsBehindHead(ply) end, "Руки вверх (обычно)" },
                        })
                    else
                        hg.CreateRadialMenu({
            ...
            inHandsBehindHead and "Сдаюсь (Руки за головой)\nПКМ - Варианты" or "Сдаюсь\nПКМ - Варианты"
        }
    else
        local wep = ply:GetActiveWeapon()
