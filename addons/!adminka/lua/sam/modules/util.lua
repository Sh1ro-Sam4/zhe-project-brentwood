...
command.new("adminmode")
	...
	:OnExecute(function(ply)
        if ply:GetNWBool("sam_adminmode", false) then
            for _, wepClass in ipairs(admitems) do
                if ply:HasWeapon(wepClass) then
                    ply:StripWeapon(wepClass)
                end
            end

            ply:SetNWBool("sam_adminmode", false)
            return
        end
        ...
        ply:SetNWBool("sam_adminmode", true)

		sam.player.send_message(nil, "adminmode", {
			A = ply
		})
	end)
:End()
...
do
	...
	hook.Add("PlayerNoClip", "SAM.CanNoClip", function(ply)
		if ply:HasPermission("can_noclip") then
			if SERVER then
				mLogs.log("samcommand", "sam", {player1=mLogs.logger.getPlayerData(ply),command="noclip",args=""})
			end
...
command.new("give")
	...
	:OnExecute(function(ply, targets, weapon)
		for i = 1, #targets do
			local wep = targets[i]:Give(weapon)
			wep.UnDroppable = true
		end
