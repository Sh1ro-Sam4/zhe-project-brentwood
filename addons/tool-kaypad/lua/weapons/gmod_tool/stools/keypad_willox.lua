if (SERVER) then
	CreateConVar('sbox_maxkeypads', 5)
end
...
TOOL.ClientConVar['password'] = ("%s%s%s%s"):format(math.random(1,9),math.random(1,9),math.random(1,9),math.random(1,9))
...
if CLIENT then
	...
	language.Add("Undone_Keypad", "Убираем keypad")
	language.Add("Cleanup_keypads", "Keypads")
	language.Add("Cleaned_keypads", "Keypad очищены")

	language.Add("SBoxLimit_keypads", "У тебя лимит keypad!")
end
...
function TOOL:RightClick()
	...
	-- 	ply:PrintMessage(3, "Некорректный пароль!")
	-- 	return false
	-- end
	--
...
function TOOL:LeftClick(tr)
	...
	if CLIENT then 
		self.next_nessage_secure = self.next_nessage_secure or 0
		if CurTime() > self.next_nessage_secure and !tobool(self:GetClientNumber("secure")) then
			chat.AddText(Color(0,200,0),"[КейПад] ",Color(255,0,0),"Внимание!",Color(255,255,255), " Вы не активировали ",
			Color(0,255,0),"\"Скрытые цифры\"",Color(255,255,255)," в настройках кейпада. Посторонние лица могут увидеть пароль")
			self.next_nessage_secure = CurTime() + 5
		end
		return true 
	end
...
if CLIENT then
	...
	function TOOL.BuildCPanel(CPanel)
		...
		CPanel:ControlHelp("Макс цифр: 4\nРазрешеные цифры: 1-9")

		CPanel:CheckBox("Скрытые цифры", "keypad_willox_secure")
		CPanel:CheckBox("Приварить", "keypad_willox_weld")
		CPanel:CheckBox("Заморозить", "keypad_willox_freeze")
		...
		local ctrl = vgui.Create("CtrlNumPad", CPanel)
			...
			ctrl:SetLabel1("Кнопка доступа")
			ctrl:SetLabel2("Кнопка запрета")
...
--]]
		CPanel:Button("Сбросить настройки", "keypad_willox_reset")
