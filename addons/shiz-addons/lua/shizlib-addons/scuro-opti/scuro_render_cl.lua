...
local max_distance_sqr = 1000 ^ 2
local function IsTargetEntity(ent)
    local class = ent:GetClass()
    
    -- local is_prop = (class == "prop_physics" or class == "prop_dynamic")
    local is_prop = false
    local is_door = class:lower():find("door") ~= nil
    
    return is_prop or is_door
end

local function optimize_entity(ent)
    if not IsValid(ent) then return end
    
    if ent:IsPlayer() or ent:IsNPC() then return end
    
    if not IsTargetEntity(ent) then return end

    local timer_id = "fps_opt_" .. ent:EntIndex()
    timer.Create(timer_id, math.Rand(0.8, 1.2), 0, function()
        ...
        if ply:GetPos():DistToSqr(ent:GetPos()) > max_distance_sqr then
...
hook.Remove("NetworkEntityCreated", "PrimitiveFPSOptimizer", function(ent)
    timer.Simple(0.1, function()
        optimize_entity(ent)
    end)
end)

hook.Remove("InitPostEntity", "FPSOptimizerInit", function()
...
do
	...
	CreateConVar('svc_propmax', '500', fl, '', 500, 8000)
	CreateConVar('svc_playermin', '3000', fl, '', -1, 8000)
	CreateConVar('svc_playermax', '3000', fl, '', 500, 8000)
	CreateConVar('svc_npcmin', '1000', fl, '', -1, 8000)
	CreateConVar('svc_npcmax', '1000', fl, '', 500, 8000)
	CreateConVar('svc_vehiclemin', '-1', fl, '', -1, 8000)
	CreateConVar('svc_vehiclemax', '100', fl, '', 500, 8000)
