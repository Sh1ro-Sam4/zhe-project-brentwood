...
function CITYWORKER.Begin( ply, ent )
    if ent.CW_isWorked then
        notif(ply, CITYWORKER.Config.Language["JOB_WORKED"] )
        return
    end

    if ply.CW_ent != ent then
        notif(ply, CITYWORKER.Config.Language["ASSIGNED_ELSE"] )
        return
    end
...
function CITYWORKER.Finish( ply, ent )
    ...
    if not con:HasItem('shizlib_resource_tools') then return CITYWORKER.Stop( ply, ent ) end
    con:TakeItems('shizlib_resource_tools', 1)

    local pay = CITYWORKER.Config[CITYWORKER_DEFINITIONS[ent:GetClass()][1]].Payout
    ply:GiveSalary( pay )
    hook.Call('workerxyerker', nil, ply)

    CITYWORKER.Stop( ply, ent )
    CITYWORKER.Remove( ply )

    -- local str = string.Replace( CITYWORKER.Config.Language["PAYOUT"], "%s", shizlib.FormatMoney( pay ) )
    -- notif(ply, str )
end
...
function CITYWORKER.Cancel( ply, ent )
    ...
    notif(ply, CITYWORKER.Config.Language["CANCELLED"] )
end
...
function CITYWORKER.Assign( ply, job, id )
    ...
    notif(ply, CITYWORKER.Config.Language["NEW_JOB"] )
    ply:EmitSound("sound/phone/ringtone.mp3")
end
...
function CITYWORKER.Initialize()
    ...
    --     end
    -- end )
    timer.Create('CityWorker.GiveTasks', 60, 0, function()
        for k, pl in player.Iterator() do
            if pl.CW_ent then continue end
            if not pl.Inventory then continue end
            local id = pl.Inventory:GetID()
            local con = itemstore.containers.Get( id )

            if con:HasItem('shizlib_resource_tools') then
                if pl.CW_ent then continue end
                CITYWORKER.Give(pl)
                timer.Create( "CityWorker.RemoveTask" .. pl:SteamID64(), 180, 1, function()
                    if IsValid(pl) then
                        CITYWORKER.Remove( pl )
                    end
                end )
            end
        end
...
hook.Add('PostCleanupMap', 'CITYWORKER.PostCleanupMap', CITYWORKER.Initialize)
...
function CITYWORKER.SendData( ply )
    ...
    if not ply:IsSuperAdmin() then
        AddLegacy( "You must be a superadmin to use the configurator tool!", NOTIFY_ERROR, 4 )
...
net.Receive( "CITYWORKER.Add", function( len, ply )
    ...
    if #CITYWORKER_DATA[game.GetMap()] > 255 then 
        notif(ply, "The maximum number of city worker positions is 255!" )
        return
    end
    ...
    ply:Notify("Successfully added new city worker task!" )
end )
...
net.Receive( "CITYWORKER.Remove", function( len, ply )
    ...
    ply:Notify("Successfully removed a city worker task!" )
end )
...
local CW_tbl = {
    ['cityworker_electric'] = true,
    ['cityworker_rubble'] = true,
    ['cityworker_leak'] = true,
    ['cityworker_hydrant'] = true,
}

hook.Add('PlayerUse', 'CW_Use', function(ply, ent)
    if not IsValid(ply) then return end
    if ply.CDUse == nil then ply.CDUse = CurTime() + 3 end
    if ply.CDUse > CurTime() then return end

    if not CW_tbl[ent:GetClass()] then return end
    local id = ply.Inventory:GetID()
	local con = itemstore.containers.Get( id )
    if not con:HasItem('shizlib_resource_tools') then return DarkRP.notify(ply, 1, 4, 'У вас нет необходимых инструментов!') end

    CITYWORKER.Begin( ply, ent )
    ply.CDUse = CurTime() + 3
end)