...
local function OnInitialize()
	for key,value in pairs(condefs) do
		CreateConVar(key, value, (FCVAR_ARCHIVE))
	end
...
local function OnPaintHUD()
	if LocalPlayer():IsValid() and LocalPlayer():GetActiveWeapon():IsValid() and (LocalPlayer():GetActiveWeapon():GetClass() != 'gmod_tool') then target.active = false return end
	target.active = false
